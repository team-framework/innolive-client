...
- 온디바이스 모드에서는 얼굴과 번호판을 모두 비식별화한다. 기기 얼굴 관리에서 등록한 얼굴만 2회 연속 인식 뒤 해당 송출 프레임에서도 확인된 경우 블러 예외가 될 수 있다. 번호판은 항상 보호한다. 서버 얼굴 목록과 기기 목록은 분리한다.
...
직전 YOLO 위치는 crop 선택에만 사용한다. 현재 crop의 YuNet 얼굴 위치와 현재
YOLO 얼굴 위치의 IoU 0.5 이상, 같은 track·등록자, 2회 확인, 정확히 같은 capture
시각의 결과가 모두 맞아야 그 프레임에서 예외를 허용한다. crop은 원본과 독립된
픽셀을 소유하고 작업자가 해제한다. 회전·크기·카메라·모드·등록 목록 세대가 바뀌면
오래된 결과를 무효화한다.

750ms 이력은 재확인할 후보 선택과 연속 확인에 사용한다. iOS의 과거 결과만으로
최대 750ms 예외를 유지하는 정책과는 다르다. 같은 위치에서 사람을 교체하는 이전
P1 요구를 유지하기 위해 늦은 결과나 확인되지 않은 현재 프레임은 블러를 유지한다.
따라서 CPU 대체 경로나 빠르게 움직이는 얼굴에서는 등록 얼굴도 자주 블러될 수 있다.
현재 프레임 결과가 YOLO 종료 전에 도착해야 예외가 적용되며, 모든 프레임에서 예외가
유지되거나 iOS와 같은 화면을 보장하지 않는다.
...
실기기 테스트 8개 통과. 비동기 테스트는 멈춘 작업이 송출을 기다리게 하거나 작업을
