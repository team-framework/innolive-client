...
- 온디바이스 모드에서는 얼굴과 번호판을 모두 비식별화한다. 기기 얼굴 관리에서 등록한 얼굴만 2회 연속 인식 뒤 최대 500ms 동안 블러 예외가 될 수 있다. 최소 250ms 간격으로 재확인한다. 번호판은 항상 보호한다. 서버 얼굴 목록과 기기 목록은 분리한다.
...
아래는 GPU·캐시 후속 변경 전 기록이다. 당시에는 등록 얼굴 예외 후보의 동기 현재 프레임 재확인을 한 송출 처리 흐름에서 수행했다.
...
직전 YOLO 위치는 crop 선택에 사용한다. crop에서 실제 인식한 YuNet 얼굴의 위치와
현재 YOLO 트랙의 IoU 0.5 이상 및 세대를 확인한 뒤 매칭 결과를 반영한다. 두 번
확인된 등록자의 결과는 프레임 시각부터 최대 500ms 동안 다음 프레임에도 재사용한다.
최소 250ms 간격으로 오래 대기한 얼굴부터 재확인한다. crop은 원본과 독립된 픽셀을
소유하고 작업자가 해제한다. 회전·크기·카메라·모드·등록 목록 세대가 바뀌면 오래된
결과를 무효화한다.

초기 GPU 변경의 엄격한 현재 프레임 확인 정책은 사용자 승인 후 iOS와 같은 짧은
예외 캐시 정책으로 변경했다. 현재 프레임 결과가 YOLO 종료 전에 도착할 필요가 없으며,
불일치·모호한 얼굴·트랙 소실은 즉시 취소한다. 결과가 늦거나 샘플이 없으면 기존
500ms 기한을 연장하지 않고, 기한이 지나면 블러로 복귀한다. 같은 위치에서 사람이
바뀌면 새 결과 또는 기존 기한까지 예외가 이어질 수 있다. 두 플랫폼의 공통 기준은
[얼굴 예외 캐시 정책](on-device-face-exceptions.md)을 참고한다.
...
실기기 테스트 8개 통과(GPU 최초 변경 기준). 당시 엄격한 현재 프레임 예외 검사는
아래 500ms 캐시 후속 정책으로 대체했다. 최신 테스트 수와 검증은 공통 정책 문서를 참고한다.
