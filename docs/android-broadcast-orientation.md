...
일시적인 `DISCONNECTED`에서는 서버 설정의 디바운스 시간 동안 기존 연결의 자연 회복을 기다린다. 회복되지 않으면 같은 세션과 PeerConnection에서 ICE restart를 시도하며 방향 잠금, CameraX use case와 방송 상태를 유지한다. 이전 협상의 늦은 answer·후보는 협상 ID로 무시한다. 복구 창 또는 시도 횟수를 소진해 최종 실패로 전환될 때 잠금을 해제한다. 사용자의 방송 종료·로그아웃도 복구 작업을 취소하고 잠금을 해제한다.
