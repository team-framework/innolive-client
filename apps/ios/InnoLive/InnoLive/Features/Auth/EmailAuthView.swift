...
struct EmailAuthView: View {
    ...
    private var title: String {
        switch step {
        case .signIn: String(localized: "이메일로 로그인")
        case .signUp: String(localized: "계정 만들기")
        case .verification: String(localized: "이메일을 확인해 주세요")
        }
    }
    private var actionTitle: String {
        switch step {
        case .signIn: String(localized: "로그인")
        case .signUp: String(localized: "인증 메일 보내기")
        case .verification: String(localized: "인증하고 시작하기")
        }
    }
        ...
        .toolbar {
            if step != .signIn {
                ToolbarItem(placement: .topBarLeading) {
                    Button(String(localized: "뒤로"), systemImage: "chevron.left", action: goBack)
                        .disabled(authentication.isLoading)
                }
            }
            ToolbarItemGroup(placement: .keyboard) {
                Spacer()
                Button(String(localized: "완료")) { focusedField = nil }
            }
        }
    ...
    private var header: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            if step == .verification {
                ...
                Text(String(localized: "위 주소로 보낸 6자리 인증 코드를 입력해 주세요."))
                    .foregroundStyle(.secondary)
            } else {
                Text(step == .signIn ? String(localized: "InnoLive에서 라이브를 이어가세요.") : String(localized: "이메일 인증 후 라이브를 시작할 수 있어요."))
                    .foregroundStyle(.secondary)
            }
        }
    }
    ...
    private var credentials: some View {
        VStack(alignment: .leading, spacing: 20) {
            VStack(alignment: .leading, spacing: 8) {
                inputField(String(localized: "이메일"), field: .email) {
                    TextField(String(localized: "이메일"), text: $email, prompt: Text(verbatim: "name@example.com"))
                        ...
                        .accessibilityLabel(String(localized: "이메일"))
                }
                if editedFields.contains(.email), !email.isEmpty, !hasValidEmail {
                    feedback(String(localized: "이메일 주소를 확인해 주세요."), symbol: "exclamationmark.circle", color: .red)
                }
            }
            VStack(alignment: .leading, spacing: 8) {
                passwordField(String(localized: "비밀번호"), text: $password, field: .password)
                if step == .signUp {
                    feedback(
                        password.isEmpty ? String(localized: "영문·숫자 기준 8~72자") : hasValidPassword ? String(localized: "사용할 수 있는 비밀번호입니다.") : String(localized: "영문·숫자 기준 8~72자로 입력해 주세요."),
            ...
            if step == .signUp {
                VStack(alignment: .leading, spacing: 8) {
                    passwordField(String(localized: "비밀번호 확인"), text: $passwordConfirmation, field: .passwordConfirmation)
                    if !passwordConfirmation.isEmpty {
                        if password == passwordConfirmation {
                            feedback(String(localized: "비밀번호가 일치합니다."), symbol: "checkmark.circle.fill", color: .green)
                        } else if editedFields.contains(.passwordConfirmation) {
                            feedback(String(localized: "비밀번호가 일치하지 않습니다."), symbol: "exclamationmark.circle", color: .red)
                        }
                    }
                }
            }
    ...
    private var verificationField: some View {
        inputField(String(localized: "인증 코드"), field: .verificationCode) {
            TextField(String(localized: "6자리 숫자"), text: $verificationCode)
    ...
    @ViewBuilder private var footer: some View {
        VStack(spacing: 4) {
            if step == .verification {
                Text(didResendCode ? String(localized: "인증 코드를 다시 보냈어요.") : String(localized: "메일이 오지 않았다면 스팸함을 확인해 주세요."))
                ...
                Button {
                    Task {
                        ...
                        if didResendCode {
                            verificationCode = ""
                            UIAccessibility.post(notification: .announcement, argument: String(localized: "인증 코드를 다시 보냈어요."))
                        }
                    }
                } label: {
                    Text(String(localized: "인증 코드 다시 보내기")).frame(minHeight: 44)
                }
            } else {
                Text(step == .signIn ? String(localized: "InnoLive가 처음이신가요?") : String(localized: "이미 계정이 있으신가요?"))
                    .font(.subheadline).foregroundStyle(.secondary)
                Button {
                    changeStep(to: step == .signIn ? .signUp : .signIn)
                } label: {
                    Text(step == .signIn ? String(localized: "회원가입") : String(localized: "로그인")).frame(minWidth: 44, minHeight: 44)
                }
            }
        }
    ...
    private func passwordField(_ title: String, text: Binding<String>, field: Field) -> some View {
        inputField(title, field: field) {
            HStack(spacing: 8) {
                ZStack {
                    SecureField(String(localized: "비밀번호 입력"), text: text)
                    ...
                    TextField(String(localized: "비밀번호 입력"), text: text)
                ...
                .buttonStyle(.plain)
                .accessibilityLabel(visiblePasswords.contains(field)
                    ? String(localized: "\(title) 가리기")
                    : String(localized: "\(title) 보기"))
            }
        }
    }
